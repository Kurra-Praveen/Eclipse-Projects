...
public class ExcelData {
	 ...
	 * @param row
	 * @param columm
	 * @return the specific cell data from the excel by passing row number and
	 *         column number
	 */
	...
	public String getDataByKey(String sheet, String key) {

		int column = getColumnCount(sheet);
		int rows = getRowCount(sheet);

		for (int i = 1; i < rows; i++) {
			if (getstringdata(sheet, i, 0).equals(key)) {

				return getstringdata("Sheet1", i, column - 1);
